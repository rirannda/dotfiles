...
case "$MODE" in
  window)
    # window capture: fall back to area selection if no active-window helper
    ...
    if [ -n "$GEOM" ]; then
      ...
      if grim -g "$GEOM" "$FILE"; then
        ...
        swappy -f "$FILE" &
        notify_cmd "Saved $FILE (copied to clipboard)"
      else
  ...
  full|"")
    ...
    ;;
  *)
    echo "Usage: $0 [full|window|area]" >&2
    exit 2
    ;;
esac
