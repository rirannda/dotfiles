...
Rectangle {
    ...
    Process {
        ...
        command: ["sh", "-c", "if command -v nvidia-smi > /dev/null; then nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits 2>/dev/null || echo 'N/A'; elif [ -e /sys/class/drm/card0/device/gpu_busy_percent ]; then cat /sys/class/drm/card0/device/gpu_busy_percent 2>/dev/null || echo 'N/A'; elif lspci 2>/dev/null | grep -i 'vga\|3d\|gpu' | grep -iq 'intel\|amd'; then echo '0'; else echo 'N/A'; fi"]
        running: true

        stdout: SplitParser {
            onRead: (data) => {
                var str = data.toString().trim();
                if (str === "N/A" || str === "" || str === "none") {
                    root.gpuUsage = 0;
                    root.gpuStatus = "N/A";
                } else {
                    var val = parseInt(str) || 0;
                    root.gpuUsage = val;
                    root.gpuStatus = val + "%";
                }
            }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (data) => {
                if (data) {
                    let tempVal = parseInt(data.trim()) || 0;
                    root.cpuTemp = Math.round(tempVal / 1000);
                }
            }
        }
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "if [ -e /sys/class/thermal/thermal_zone1/temp ]; then cat /sys/class/thermal/thermal_zone1/temp; elif [ -e /sys/class/drm/card0/device/gt_cur_freq_mhz ]; then echo '0'; else echo '0'; fi"]

        stdout: SplitParser {
            onRead: (data) => {
                if (data) {
                    let tempVal = parseInt(data.trim()) || 0;
                    root.gpuTemp = Math.round(tempVal / 1000);
                }
            }
        }
    }
    ...
    RowLayout {
        ...
        CircularGauge {
            label: "󰢮 GPU"
            value: root.gpuUsage ? root.gpuUsage : ""
            accentColor: root.gpuUsage ? Theme.red : Theme.muted
            sublabel: root.gpuTemp ? root.gpuTemp + "°C" : root.gpuStatus
        }
    ...
    component CircularGauge: Item {
        ...
        ThemedText {
            ...
            anchors.verticalCenterOffset: -Constants.sizeSm
        ...
        ThemedText {
            ...
            text: sublabel
            visible: sublabel !== ""
            color: sublabel !== "N/A" ? accentColor : Theme.muted
